...
def load_dataset(img_height, img_width):
    ...
    nonblinkpath = "./src/resources/blinkset/nonblinkers"
    blinkpath = "./src/resources/blinkset/blinkers"
    
    all_x = []
    all_y = []
    img_count = 0
    # Load all images from lfw with label in all_x and all_y
    #for folder in os.listdir(nonblinkpath):
    ...
    #for folder in os.listdir(blinkpath):
    ...
    print("Finished loading all images! Loaded " + str(img_count) + " images!\n")
    return all_x, all_y
...
def train(all_x, all_y, split, device):
    ...
    batch_size = 64
    epoch = 11
    model_path = "./src/backend/saved_models/blinkingFaces.pth"
    ...
    print(str(train_dataset.__len__))

    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False)

    model = cropped_model().to(device)
    criteria = nn.BCELoss()
    ...
    for epoch_num in range(epoch):
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i+1) % 20 == 0:
                print (f"Epoch: [{epoch_num+1}/{epoch}], Step: [{i+1}/{n_total_steps}], Loss: {loss.item():.4f}")
